...
const MediaModal = ({ newMedia, setNewMedia, setShowModal, setUpdate, update, id, fetchMedia}) => {
    ...
    const handleNewMediaImage = async (e) => {
    ...
    const handleModalSubmit = async (e) => {
        ...
        try {
            ...
            if (update) {
                ...
                data = await res.json();
                setPopup({ show: true, message: data.message || "Media updated!", type: data.success ? "success" : "error" });  
                await fetchMedia();
                setUpdate(false);
            } else {
                ...
                data = await res.json();
                setPopup({ show: true, message: data.message || "Media added!", type: data.success ? "success" : "error" });
                await fetchMedia();
            }
        } catch (err) {
            ...
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl bg-white p-0 border border-blue-100 relative animate-fadeIn">
                ...
                >
                    ...
                    <h2 className="text-2xl font-bold mb-6 text-blue-700 flex items-center gap-2">
                        ...
                        {update ? "Update Media" : "Add New Media"}
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <textarea
                                ...
                                onChange={handleNewMediaChange}
                                required
                                className="w-full border border-blue-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 px-3 py-2 rounded-lg mb-4 transition resize-none text-gray-600"
                                rows={4}
                                placeholder="Brief Media description"
                            />
                        </div>
                        <div>
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                <label className="flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-blue-300 rounded-lg cursor-pointer hover:border-blue-500 transition">
                                    ...
                                    <input
                                        type="file"
                                        accept="image/*"
                                        onChange={handleNewMediaImage}
                                        className="hidden"
                                    />
                                </label>
                                {newMedia.image && (
                                    <img src={newMedia.image} alt="Preview" className="h-24 w-24 rounded-lg object-cover border border-blue-200 shadow" />
                                )}
                            </div>
                        </div>
                    </div>
